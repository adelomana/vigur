...
for (experiment in experiments){
  print(paste('working with experiment', experiment))
  
  ### define first the time contrasts without treatments
  influence = 'time'

  reference_samples = metadata[metadata$run == run & metadata$treatment == 'zero' & metadata$time == 'zero' & metadata$experiment == experiment, ]$sampleID
  testing_samples = metadata[metadata$run == run & metadata$treatment == 'zero' & metadata$time == 'four' & metadata$experiment == experiment, ]$sampleID
  ...
  the_tag = paste('run', run, 'experiment', experiment, influence, 'four_vs_zero', sep='_')
  ...
  reference_samples = metadata[metadata$run == run & metadata$treatment == 'zero' & metadata$time == 'zero' & metadata$experiment == experiment, ]$sampleID
  testing_samples = metadata[metadata$run == run & metadata$treatment == 'zero' & metadata$time == 'twentyfour' & metadata$experiment == experiment, ]$sampleID
  ...
  the_tag = paste('run', run, 'experiment', experiment, influence, 'twentyfour_vs_zero', sep='_')
  hypo = list('reference'=reference_samples, 'testing'=testing_samples, 'influence'=influence, 'tag'=the_tag)
  print(hypo)
  hypos = list.append(hypos, hypo)
  print('----------------------------------------------------------')
  ...
  for (time in timepoints){
    ...
    for (treatment in treatments){
      reference_samples = metadata[metadata$run == run & metadata$treatment == 'zero' & metadata$time == time & metadata$experiment == experiment, ]$sampleID
      testing_samples = metadata[metadata$run == run & metadata$treatment == treatment & metadata$time == time & metadata$experiment == experiment, ]$sampleID
      ...
      the_tag = paste('run', run, 'experiment', experiment, 'treatment', treatment, 'time', time, sep='_')
      hypo = list('reference'=reference_samples, 'testing'=testing_samples, 'influence'=influence, 'tag'=the_tag)
      print(hypo)
      hypos = list.append(hypos, hypo)
      print('----------------------------------------------------------')
