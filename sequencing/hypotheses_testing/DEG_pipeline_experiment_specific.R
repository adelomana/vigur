#
# This script performs experiment-specific comparisons for 072 and 073 approaches.
...
for (experiment in experiments){
  ...
  rule = metadata$run == run & metadata$treatment == 'zero' & metadata$time == 'zero' & metadata$experiment == experiment
  reference_samples = metadata[rule, ]$sampleID
  rule = metadata$run == run & metadata$treatment == 'zero' & metadata$time == 'four' & metadata$experiment == experiment
  testing_samples = metadata[rule, ]$sampleID
  ...
  the_tag = paste('run', run, influence, 'four_vs_zero', 'exp', experiment, sep='_')
  ...
  rule = metadata$run == run & metadata$treatment == 'zero' & metadata$time == 'twentyfour' & metadata$experiment == experiment
  testing_samples = metadata[rule, ]$sampleID
  ...
  the_tag = paste('run', run, influence, 'twentyfour_vs_zero', 'exp', experiment, sep='_')
  hypo = list('reference'=reference_samples, 'testing'=testing_samples, 'influence'=influence, 'tag'=the_tag)
  print(hypo)
  hypos = list.append(hypos, hypo)
  print('----------------------------------------------------------')
...
for (time in timepoints){
  for (treatment in treatments){
    for (experiment in experiments){
      ...
      rule = metadata$run == run & metadata$treatment == 'zero' & metadata$time == time & metadata$experiment == experiment
      reference_samples = metadata[rule, ]$sampleID
      rule = metadata$run == run & metadata$treatment == treatment & metadata$time == time & metadata$experiment == experiment
      testing_samples = metadata[rule, ]$sampleID
      ...
      the_tag = paste('run', run, 'treatment', treatment, 'time', time, 'exp', experiment, sep='_')
      hypo = list('reference'=reference_samples, 'testing'=testing_samples, 'influence'=influence, 'tag'=the_tag)
      print(hypo)
      hypos = list.append(hypos, hypo)
      print('----------------------------------------------------------')
...
for (time in timepoints){
  for (treatment in treatments){
    for (experiment in experiments){
      ...
      # making sure that 073_39 is not used
